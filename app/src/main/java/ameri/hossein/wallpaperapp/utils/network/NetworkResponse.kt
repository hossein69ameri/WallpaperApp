package ameri.hossein.wallpaperapp.utils.network

import ameri.hossein.wallpaperapp.data.model.ResponseErrors
...
open class NetworkResponse<T>(private val response: Response<T>) {
    ...
    open fun generateResponse(): NetworkRequest<T> {
        return when {
            ...
            response.code() == 403 -> NetworkRequest.Error("Rate Limit Exceeded")
            response.code() == 422 -> {
                var errorMessage = ""
